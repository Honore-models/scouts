          ...
          <main className="px-6 py-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-[14px] text-[#666]">Continue working on your unpublished projects</p>
              </div>
              <Link href="/launch" className="flex items-center gap-2 rounded-xl bg-[#315BFF] px-4 py-2.5 text-[13px] font-semibold text-white shadow-[0_2px_8px_rgba(49,91,255,0.25)] hover:bg-[#254DE8]">
                <Plus className="h-4 w-4" />
                New Draft
              </Link>
            </div>
            ...
            {items.length > 0 ? (
              <div className="mt-6 space-y-3">
                {items.map((draft) => (
                  <div key={draft.id} className="group flex items-center gap-4 rounded-xl bg-white/60 p-4 backdrop-blur-sm transition-all duration-200 hover:bg-white hover:shadow-[0_4px_16px_rgba(0,0,0,0.06)]">
                    {/* Image */}
                    <div className="relative h-[80px] w-[120px] shrink-0 overflow-hidden rounded-lg bg-slate-900">
                      <Image src={draft.image} alt={draft.name} fill className="object-cover" sizes="120px" />
                    </div>

                    {/* Content */}
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <h3 className="text-[15px] font-semibold text-[#111]">{draft.name}</h3>
                        <span className="rounded-md bg-[#F3F4F6] px-2 py-0.5 text-[10px] font-medium text-[#666]">{draft.category}</span>
                      </div>
                      <p className="mt-1 text-[13px] text-[#666] truncate">{draft.description}</p>
                    ...
                    <div className="w-[140px] shrink-0">
                      <div className="flex items-center justify-between text-[11px] text-[#666] mb-1">
                        ...
                        <span className="font-medium">{draft.completionPercent}%</span>
                      </div>
                    ...
                    <div className="flex items-center gap-1 shrink-0">
                      <Link href="/launch" className="rounded-lg p-2 text-[#555] hover:bg-[#315BFF]/10 hover:text-[#315BFF] transition-colors">
                        <Edit3 className="h-4 w-4" />
                      </Link>
                      <button type="button" onClick={() => removeItem(draft.id)} className="rounded-lg p-2 text-[#999] hover:bg-red-50 hover:text-[#EF4444] transition-colors">
              ...
              <div className="mt-12 text-center">
                ...
                <p className="mt-3 text-[16px] font-semibold text-[#111]">No drafts</p>
                <p className="mt-1 text-[13px] text-[#666]">Start a new project to see drafts here</p>
                <Link href="/launch" className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#315BFF] px-5 py-2.5 text-[13px] font-semibold text-white hover:bg-[#254DE8]">
                  <Plus className="h-4 w-4" />
                  Ship a product
                </Link>
              </div>
