...
const allProducts = [
  { id: '1', name: 'FlowBoard', tagline: 'AI powered whiteboard for collaborative teams', maker: 'David Kim', image: '/landing/dashboard-tilt.png', category: 'SaaS', upvotes: 482, comments: 68, rating: 4.8 },
  { id: '2', name: 'DeliciousFood', tagline: 'All in one platform that helps restaurants grow from dashboard to doorstep', maker: 'Ivan', image: '/landing/delicious.png', category: 'Web Apps', upvotes: 341, comments: 45, rating: 4.7 },
  { id: '3', name: 'CodeSnap', tagline: 'Beautiful code screenshots in one click', maker: 'Cenat', image: '/landing/code.png', category: 'Developer tools', upvotes: 298, comments: 32, rating: 4.6 },
  { id: '4', name: 'Kartz', tagline: 'Art Selling website for independent artists', maker: 'Mandrake', image: '/landing/kartz.png', category: 'Marketplace', upvotes: 267, comments: 28, rating: 4.5 },
  { id: '5', name: 'ShipFast', tagline: 'Ship your MVP in days not months', maker: 'Liam Chen', image: '/landing/code.png', category: 'Developer tools', upvotes: 234, comments: 22, rating: 4.4 },
  { id: '6', name: 'MindMap AI', tagline: 'AI powered mind mapping and brainstorming', maker: 'Alex Park', image: '/landing/delicious.png', category: 'AI', upvotes: 198, comments: 18, rating: 4.3 },
  { id: '7', name: 'LandingKit', tagline: 'Beautiful landing page templates for startups', maker: 'Sara Kim', image: '/landing/kartz.png', category: 'Design', upvotes: 167, comments: 15, rating: 4.2 },
  { id: '8', name: 'DataPulse', tagline: 'Real time analytics dashboard for SaaS', maker: 'Mike Ross', image: '/landing/code.png', category: 'SaaS', upvotes: 145, comments: 12, rating: 4.1 },
  { id: '9', name: 'PixelForge', tagline: 'AI image generation and editing platform', maker: 'Emma Wilson', image: '/landing/delicious.png', category: 'AI', upvotes: 123, comments: 9, rating: 4.0 },
];

const filterOptions = ['All', 'SaaS', 'AI', 'Developer tools', 'Web Apps', 'Mobile', 'Design', 'Marketplace'];
...
export default function SearchPage() {
  ...
  const results = allProducts.filter(p => {
    const matchesQuery = query === '' || p.name.toLowerCase().includes(query.toLowerCase()) || p.tagline.toLowerCase().includes(query.toLowerCase());
      ...
      <div className="flex min-h-screen">
        ...
        <div className="flex-1">
          ...
          <main className="px-6 py-6">
            ...
            <p className="mt-1 text-[14px] text-[#666]">Find projects, developers, and tools</p>
            ...
            <div className="mt-5 relative">
              ...
              />
              {query && (
                <button type="button" onClick={() => setQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#999] hover:text-[#555]">
                  <X className="h-4 w-4" />
                </button>
              )}
            </div>
            ...
            <div className="mt-4 flex items-center gap-1.5 overflow-x-auto">
              ...
              {filterOptions.map((filter) => (
                <button key={filter} type="button" onClick={() => setActiveFilter(filter)} className={`shrink-0 rounded-lg px-3 py-1.5 text-[12px] font-medium transition-colors duration-150 ${activeFilter === filter ? 'bg-[#315BFF] text-white' : 'bg-white/60 text-[#555] hover:bg-white hover:text-[#111]'}`}>
                  {filter}
                </button>
              ))}
            </div>
            ...
            {/* Results Count */}
            <p className="mt-4 text-[13px] text-[#666]">{results.length} result{results.length !== 1 ? 's' : ''} found</p>
            ...
            {results.length === 0 && (
              <div className="mt-12 text-center">
                ...
                <p className="mt-3 text-[16px] font-semibold text-[#111]">No results found</p>
                <p className="mt-1 text-[13px] text-[#666]">Try a different search term or filter</p>
              </div>
            )}
          </main>
        </div>
