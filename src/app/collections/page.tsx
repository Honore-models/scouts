          ...
          <div className="sticky top-0 flex h-screen flex-col px-4 py-6">
            ...
            <nav className="space-y-1">
              {sidebarNav.map((item) => {
                ...
                return (
                  <Link key={item.label} href={item.href} className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-[14px] font-medium transition-colors duration-150 $'text-[#555] hover:bg-black/[0.03] hover:text-[#111]'`}>
        ...
        <div className="flex-1">
          ...
          <main className="px-6 py-6">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-[28px] font-bold text-[#111]">Collections</h1>
                <p className="mt-1 text-[14px] text-[#666]">Organize projects into curated collections</p>
              </div>
              <button
                type="button"
            ...
            <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
              {collections.map((collection) => (
                <div key={collection.id} className="group rounded-xl border border-black/[0.04] bg-white/60 backdrop-blur-sm overflow-hidden transition-all duration-200 hover:bg-white hover:shadow-[0_4px_16px_rgba(0,0,0,0.06)]">
                  {/* Image */}
                  <div className="relative h-[120px] w-full overflow-hidden bg-slate-900">
                    <Image src={collection.image} alt={collection.name} fill className="object-cover" sizes="33vw" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent" />
                    <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
                      <span className="text-[13px] font-semibold text-white">{collection.projectCount} projects</span>
                  ...
                  <div className="p-4">
                    <div className="flex items-center justify-between">
                      <h3 className="text-[15px] font-semibold text-[#111]">{collection.name}</h3>
                      <button type="button" className="rounded-lg p-1 text-[#999] opacity-0 transition-opacity group-hover:opacity-100 hover:bg-black/[0.03] hover:text-[#555]">
                        <MoreHorizontal className="h-4 w-4" />
                      </button>
                    </div>
                    <p className="mt-1 text-[12px] text-[#666] line-clamp-2">{collection.description}</p>

                    {/* Project previews */}
                    <div className="mt-3 flex flex-wrap gap-1">
                      {collection.projects.slice(0, 3).map((name) => (
                        <span key={name} className="rounded-md bg-[#F3F4F6] px-2 py-0.5 text-[10px] font-medium text-[#666]">
                          {name}
                        </span>
                      ))}
