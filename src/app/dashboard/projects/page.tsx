...
export default function ProjectsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#f8f7ff] via-[#f3f1ff] to-[#e8e4ff] flex">
      {/* Sidebar */}
      <aside className="w-[220px] h-screen bg-white/60 backdrop-blur-xl border-r border-white/40 p-5 flex flex-col fixed left-0 top-0 z-10 overflow-y-auto">
        ...
        <nav className="flex-1 flex flex-col gap-1">
          ...
          {yourSpace.map((item) => (
            <Link
              key={item.label}
              href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-[14px] font-medium transition-colors ${item.active ? "bg-[#f0eeff] text-[#315BFF]" : "text-[#555] hover:bg-[#f0eeff]"}`}
            >
              <item.icon size={18} strokeWidth={1.8} />
              {item.label}
            </Link>
        ...
        <main className="flex-1 px-8 py-6">
          <div className="flex items-start justify-between mb-6">
            <div>
              ...
              <p className="text-[14px] text-[#666]">
                Manage and track all your published projects
              </p>
            </div>
            <Link
              href="/launch"
              className="flex items-center gap-2 px-5 py-2.5 bg-[#315BFF] text-white rounded-xl text-[13px] font-semibold shadow-[0_2px_8px_rgba(49,91,255,0.25)] hover:bg-[#2a4de6] transition-colors"
            >
              <Plus size={16} />
              New Project
            </Link>
          ...
          <div className="grid grid-cols-4 gap-4 mb-6">
            <div className="bg-white/50 backdrop-blur rounded-2xl p-4 border border-white/40">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-lg bg-[#f0eeff] flex items-center justify-center">
                  <Eye size={16} className="text-[#315BFF]" />
                </div>
            ...
            <div className="bg-white/50 backdrop-blur rounded-2xl p-4 border border-white/40">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-lg bg-[#f0eeff] flex items-center justify-center">
                  <Heart size={16} className="text-[#315BFF]" />
                </div>
            ...
            <div className="bg-white/50 backdrop-blur rounded-2xl p-4 border border-white/40">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-lg bg-[#f0eeff] flex items-center justify-center">
                  <MessageSquare size={16} className="text-[#315BFF]" />
                </div>
            ...
            <div className="bg-white/50 backdrop-blur rounded-2xl p-4 border border-white/40">
              <div className="flex items-center gap-2 mb-2">
                <div className="w-8 h-8 rounded-lg bg-[#f0eeff] flex items-center justify-center">
                  <Star size={16} className="text-[#315BFF]" />
                </div>
          ...
          <div className="flex items-center gap-2 mb-5">
            {["all", "live", "draft"].map((filter) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-[13px] font-medium transition-colors capitalize ${statusFilter === filter ? "bg-[#315BFF] text-white" : "bg-white/60 text-[#555] hover:bg-white"}`}
              >
                {filter}
              </button>
            ))}
          ...
          <div className="space-y-3">
            {filtered.map((project) => (
              <div
                key={project.id}
                className="flex items-center gap-5 bg-white/50 backdrop-blur rounded-2xl p-5 border border-white/40 hover:bg-white/70 transition-colors"
              >
                {/* Image */}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${project.status === "live" ? "bg-[#34C759]/10 text-[#34C759]" : "bg-[#F59E0B]/10 text-[#F59E0B]"}`}
                    >
                      {project.status}
                    </span>
                    <span className="rounded-md bg-[#f0eeff] px-2 py-0.5 text-[10px] font-medium text-[#315BFF]">
                      {project.category}
                    </span>
                  </div>
                  ...
                  <div className="mt-2 flex items-center gap-4 text-[12px] text-[#888]">
                    ...
                    <span className="flex items-center gap-1">
                      <ArrowUp size={12} className="text-[#34C759]" />
                      {project.likes}
                    </span>
                    <span className="flex items-center gap-1">
                    ...
                    <span className="flex items-center gap-1">
                      <Star size={12} className="fill-[#111]" />
                      {project.rating}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <Link
                    ...
                    className="flex items-center gap-1.5 rounded-lg border border-white/50 bg-white/60 px-3 py-2 text-[12px] font-medium text-[#555] hover:bg-white/80 transition-colors"
                  >
          ...
          {filtered.length === 0 && (
            <div className="mt-12 text-center">
              ...
              <p className="mt-1 text-[13px] text-[#666]">
                {statusFilter === "draft"
                  ? "No draft projects"
                  : "Start by shipping your first product"}
              </p>
              <Link
                href="/launch"
                className="mt-4 inline-flex items-center gap-2 rounded-xl bg-[#315BFF] px-5 py-2.5 text-[13px] font-semibold text-white hover:bg-[#2a4de6]"
              >
                <Plus size={14} />
                Ship a product
              </Link>
